...
hce-cli version    # 发布版显示 tag 版本号，源码编译显示 dev
```
...
**发布**（maintainer）：版本号锚定在根 `package.json`。`npm run release`（[bumpp](https://github.com/antfu-collective/bumpp)）一键 bump + 打 tag + 推送；推送 `v*` tag 触发 `.github/workflows/release.yml`，交叉编译 hce-cli 五平台并自动建 GitHub Release。
