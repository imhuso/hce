...
本 skill 直接驱动 `hce`，不含脚本。需要两样东西：一个可达的 **hce-server 后端**，
和本机 PATH 里的 **hce** 二进制。
...
```bash
hce config --base-url http://192.168.1.50:9528/api/v1
```

### C. 公网域名（已部署）
把 9528 反代到域名（建议 HTTPS + 鉴权）：
```bash
hce config --base-url https://hce.example.com/api/v1
```
...
4. **全局** `~/.hce/config.json` 的 `base_url`（每用户机器级默认，`hce config --base-url` 写入）
5. 内置默认 `http://localhost:9528/api/v1`

- "不同用户"：`~/.hce/` 在各自 home，天然隔离。
- "不同项目"：项目级 `.hce/config.json` 覆盖全局默认。
- 查看当前生效配置与优先级：`hce config`（无参）。
...
```bash
cd /your/project
hce status   # 看 base_url 后的「在线 / 离线」；离线先回到第 2 步改地址
```
...
hce sync     # 首次全量、之后增量
```
之后 `hce search` 会自动先增量 sync 再检索，通常无需手动 sync。
